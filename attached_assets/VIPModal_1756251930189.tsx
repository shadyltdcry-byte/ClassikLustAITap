...
export default function VIPModal({ isOpen, onClose, userId }: VIPModalProps) {
  ...
  const purchaseVipMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "VIP Purchased!",
        description: `Welcome to ${data.planType} VIP! Enjoy your exclusive benefits.`,
      });
      onClose();
    },
    onError: (error: any) => {
      toast({
        ...
        description: error.message || "Failed to purchase VIP plan",
        variant: "destructive",
      });
    },
  });
